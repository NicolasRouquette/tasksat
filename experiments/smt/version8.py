...
class TaskNetTL(TaskNetSMT):
    ...
    def check_temporal_properties(self):
        ...
        for all schedules/zone-traces that satisfy the TaskNet constraints.

        For each property φ, we build a *fresh* TaskNetTL(self.tn),
        add ¬φ(0), and see if Z3 can find a counterexample schedule+trace.
        """
        print()
        ...
        for prop in self.tn.properties:
            # Fresh encoding so we don't reuse this instance's solver/model
            enc = TaskNetTL(self.tn)
            phi = prop.formula

            # Look for a model that violates φ at position 0
            violation = Not(enc._encode_formula_at_pos(phi, 0))
            enc.solver.add(violation)
...
def make_example_tasknet() -> TaskNet:
   ...
   # Temporal properties:
    ...
    prop1 = TemporalProperty(
        name    = "battery >= 20",
        formula = TLAlways(TLNumCmp("battery", ">=", 20.0)),
    )

    prop2 = TemporalProperty(
        name    = "heating works wonders",
        formula = TLAlways(
            TLImplies(
                TLStateIs("heating", "on"),
                TLNumCmp("temperature", ">=", 200.0),

            )
        ),
    )

    prop3 = TemporalProperty(
        name    = "response: driving -> eventually communicating",
        formula = TLAlways(
            TLImplies(
                TLStateIs("driving", "on"),
                TLEventually(TLStateIs("communicating", "on")),
            )
        ),
    )
    ...
    prop4 = TemporalProperty(
        name = "heating_implies_until_communicating",
        formula = TLAlways(
            TLImplies(
                TLStateIs("heating", "on"),
                TLUntil(
                    TLNumCmp("temperature", ">=", 50.0),
                    TLStateIs("communicating", "on"),
                ),
            )
        ),
    )    
    
    prop5 = TemporalProperty(
        name = "battery_always_ge_30_sofar",
        formula = TLSoFar(TLNumCmp("battery", ">=", 30.0)),
    )
        
    prop6 = TemporalProperty(
        name = "driving_since_communicating",
        formula = TLSince(
            TLStateIs("driving", "on"),
            TLStateIs("communicating", "on"),
        ),
    ...
    return TaskNet(
        ...
        endTime   = 300,
        properties = [prop1, prop2, prop3, prop4, prop5, prop6]
    )
...
def main():
    ...
    enc = TaskNetTL(tn, error_trace=False)
