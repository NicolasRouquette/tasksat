...
class TaskNetSMT:
    ...
    def _encode_start_end_times_ok(self):
        ...
        for t in self.tn.tasks:
            ...
            # Start/end within global horizon
    ...
    def _encode_zones(self):
        ...
        zones[i] = time of zone boundary i, with:
          ...
          z_last = endTime
          z_i <= z_{i+1}
        and each task start/end coincides with some zone boundary.
        """
        n = self.tn.endTime
        z = self.zones
        ...
        # Each start/end must be equal to *some* zone time
        for t in self.tn.tasks:
            s = self.start_vars[t.id]
            e = self.end_vars[t.id]
            self.solver.add(Or(*[s == zi for zi in z]))
            self.solver.add(Or(*[e == zi for zi in z]))
    ...
    def pretty_print(self, model):
        ...
        print("\nState at each zone boundary:")
        Z = self.zone_count
        for j in range(Z):
            zj_val = model[self.zones[j]].as_long()
            print(f"\n  -- zone {j} @ time {zj_val} --")
            # State timelines
            for tl in self.tn.timelines:
                if isinstance(tl, StateTimeline):
                    _, _, i2s, vars_z = self.state_tl_zone[tl.id]
                    idx = model[vars_z[j]].as_long()
