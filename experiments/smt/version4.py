...
class TaskDef:
    ...
    endrng: IntRange
    dur: int
    start: int  # original "intended" start; we now USE this
...
class TaskNetSMT:
    ...
    def __init__(self, tn: TaskNet):
        self.tn = tn
        self.solver = Solver()
        # Hard timeout so check() cannot hang forever (10 seconds here)
        self.solver.set("timeout", 10_000)
    ...
    def _encode_start_end_times_ok(self):
        ...
        for t in self.tn.tasks:
            ...
            # **Fix the start time to the given 'start' field**:
            # this massively prunes the search space.
            self.solver.add(s == t.start)
    ...
    def solve(self):
        ...
        if res != sat:
            print(f"TaskNet constraints (schedule + trace): {res}.")
            return None
